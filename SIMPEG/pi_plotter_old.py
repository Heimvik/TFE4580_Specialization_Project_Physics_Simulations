...
class PiPlotter:
    ...
    def _quick_loglog_plot(self):
        ...
        ax.set_ylabel('emf [V]', fontsize=18)
    ...
    def plot_combined_from_hdf5(self):
        ...
        ax_decay.set_ylabel('emf [V]', fontsize=18)
    ...
    def plot_measured_vs_simulated(self):
        ...
        ax1.set_ylabel('emf [V]', fontsize=18)
    ...
    def plot_multiple_measured_and_simulated(self):
        ...
        for i, data in enumerate(measured_data):
            ...
            decay_abs = decay_abs[abs(time_shift_samples):]
            time = time[abs(time_shift_samples):]
            ...
            time_us = time * 1e6
            ax.loglog(time_us, decay_abs, linewidth=2.5, 
                     marker='o', markersize=5, markevery=max(len(time_us)//20, 1),
                     label=f"Measured decay curve (No targets)", alpha=0.8, linestyle='-')
        ...
        for i, data in enumerate(simulated_data):
            ...
            # If time shift was applied to measured data, adjust simulated to match length
            if time_shift_samples != 0 and len(measured_data) > 0:
                time_sim = time_sim[abs(time_shift_samples):]
                decay_abs = decay_abs[:-abs(time_shift_samples)]
            ...
            ax.loglog(time_us, decay_abs, linewidth=2.5,
                     ...
                     label=f"Simulated decay curve (No targets)", alpha=0.8, linestyle='-')
    ...
    def plot_conductivity_models(self):
        ...
        from pi_model_manager import PiModelManager
        
        logger = PiLogger()
        model_manager = PiModelManager(self.cfg)
        ...
        for i in range(len(labels)):
            ...
            if target_type != -1 and target_z is not None:
                type_names = {0: 'Can', 1: 'Shredded', 2: 'Block'}
                type_name = type_names.get(target_type, 'Unknown')
                status = f"{type_name} at {target_z:.2f}m"
        ...
        print(f"\nAvailable simulations:")
        for idx, (sim_idx, label, status, _, _, _) in enumerate(available_sims[:10]):
            print(f"  [{idx+1:2d}] {status}")
        
        try:
            choice = input(f"\nSelect simulation [1-{min(10, len(available_sims))}]: ").strip()
            sim_data = available_sims[int(choice) - 1]
            sim_idx, label, status, target_type, target_conductivity, target_z = sim_data
            
            model, r, z = model_manager.get_model(target_type, target_conductivity, target_z)
            
            if model is None:
                print("Model not found. Generate models first (main menu option 3).")
                return
                
            self._plot_model_2d(model, r, z, status)
            
        except (ValueError, IndexError):
            print("Invalid selection.")
    
    def _plot_model_2d(self, model, r, z, title):
        ...
        model_grid = griddata((r, z), np.log10(model), (R, Z), method='linear', 
                            fill_value=np.log10(self.cfg.air_conductivity))
        
        im = ax.pcolormesh(R, Z, model_grid, shading='auto', cmap='viridis')
        ax.set_xlabel('Radius [m]', fontsize=18)
        ax.set_ylabel('Depth [m]', fontsize=18)
        ax.set_title(f'Conductivity Model: {title}', fontsize=16)
        ax.tick_params(labelsize=16)
        ax.set_ylim([-0.4, 0.1])
        ax.set_xlim([0, 0.25])
        ax.axhline(y=0, color='brown', linewidth=3, label='Ground Surface')
        ax.legend(fontsize=14)
        
        cbar = plt.colorbar(im, ax=ax)
        cbar.set_label('log₁₀(Conductivity) [S/m]', fontsize=16)
