...
import numpy as np
from numpy import asarray, inf as Inf, isinf
try:
    # For newer SciPy versions
    from scipy.optimize._linesearch import line_search_wolfe2
    # Define our own LineSearchError since it's not available in newer versions
    class _LineSearchError(Exception):
        pass
    
    # Create a wrapper to match the old API
    def _line_search_wolfe12(f, fprime, xk, pk, gfk, old_fval, old_old_fval, **kwargs):
        ...
        # Call line_search_wolfe2 with appropriate parameters
        result = line_search_wolfe2(f, fprime, xk, pk, gfk, old_fval, old_old_fval, 
                                   c1=c1, c2=c2, amax=amax)
        
        if result[0] is None:
            raise _LineSearchError("Line search failed")
            
        # Return in the expected format: alpha, fc, gc, new_fval, new_old_fval, new_slope
        alpha_k, fc, gc, new_fval, old_old_fval, gfkp1 = result
        return alpha_k, fc, gc, new_fval, old_old_fval, gfkp1
    ...
    try:
        ...
        # Fallback: use the standard line_search function
        from scipy.optimize import line_search
        class _LineSearchError(Exception):
            pass
            
        def _line_search_wolfe12(f, fprime, xk, pk, gfk, old_fval, old_old_fval, **kwargs):
            result = line_search(f, fprime, xk, pk, gfk)
            if result[0] is None:
                raise _LineSearchError("Line search failed")
            return result[0], result[1], result[2], result[3], old_old_fval, result[4]
